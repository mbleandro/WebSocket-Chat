...
```bash
dk bash  # starts bash inside "chat" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and chat.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `chat` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The chat container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv chat -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database chat owned by a user chat.

On your remote machine, create a file ~/chat.env:

```
DJANGO_DB_PASSWORD=<chat's password>
...
server {
    ...
    location /static {
        alias /home/ubuntu/dkdata/chat/static;
...
(Replace "chat.example.com" with your production domain)
