...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development chat npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop chat
    docker rm chat
    docker run --name chat -d --env-file /home/ubuntu/chat.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/chat:/dkdata \
        chat start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./chat/
...
echo_green "Welcome to chat's dev env"
